...
public class OLD_GeneticMemory{
    ...
    public void testPaperExample() throws SBOLGraphException, IOException, SBOLValidationException, SBOLConversionException
    {
        ...
        // Component TetR_protein=SBOLAPI.createProteinComponent(doc,device, "TetR_protein", "TetR", "TetR protein", Role.TF, "NNNNNNNNNNN");
            // String label="prom..";
            
            // Component x=newDocument.createComponent(URI_prefix, null);
            // x.setName(label);
            // Sequence x_seq=newDocument.createSequence(URI_prefix, null);
            // String na=findSequence(convertedDocument, label);
            //     x.setSequences(x_seq);
            //     SubComponent sc=device.createSubComponent(x);
            //     sc.createRange(0, 0, x_seq);


            /*
            SBOLAPI.appendComponent(newDocument, device, prm_0);
            SBOLAPI.appendComponent(newDocument, device, rbs_0);
            SBOLAPI.appendComponent(newDocument, device, cds_LacI);
            SBOLAPI.appendComponent(newDocument, device, rbs_1);
            SBOLAPI.appendComponent(newDocument, device, cds_TetR);
            SBOLAPI.appendComponent(newDocument, device, ter_0);
            SBOLAPI.appendComponent(newDocument, device, prm_PTet);
            // UNKNOWN COMPONENT
            SBOLAPI.appendComponent(newDocument, device, ins_SccJ);

            SBOLAPI.appendComponent(newDocument, device, rbs_2);
            SBOLAPI.appendComponent(newDocument, device, cds_SrpR);
            SBOLAPI.appendComponent(newDocument, device, ter_1);
            SBOLAPI.appendComponent(newDocument, device, prm_PTac);
            // UNKNOWN COMPONENT
            SBOLAPI.appendComponent(newDocument, device, ins_SarJ);

            SBOLAPI.appendComponent(newDocument, device, rbs_3);
            SBOLAPI.appendComponent(newDocument, device, cds_BM3R1);
            SBOLAPI.appendComponent(newDocument, device, ter_2);
            SBOLAPI.appendComponent(newDocument, device, prm_PSrpR);
            SBOLAPI.appendComponent(newDocument, device, prm_PBM3R1);
            // UNKNOWN COMPONENT
            SBOLAPI.appendComponent(newDocument, device, ins_LtsvJ);

            SBOLAPI.appendComponent(newDocument, device, rbs_4);
            SBOLAPI.appendComponent(newDocument, device, cds_PhIF);
            // CHOICE 1
            SBOLAPI.appendComponent(newDocument, device, ter_3);
            SBOLAPI.appendComponent(newDocument, device, prm_PPhIF);
            SBOLAPI.appendComponent(newDocument, device, rbs_5);
            SBOLAPI.appendComponent(newDocument, device, cds_Int2);
            SBOLAPI.appendComponent(newDocument, device, ter_4);
            // CHOICE 2
            //SBOLAPI.appendComponent(doc, device, ter_3);
            //SBOLAPI.appendComponent(doc, device, prm_PPhIF);
            //SBOLAPI.appendComponent(doc, device, rbs_5);
            //SBOLAPI.appendComponent(doc, device, cds_YFP);
            //SBOLAPI.appendComponent(doc, device, ter_4);

            

            // Add the sequences
            /*
            for(Component comp: newDocument.getComponents()) {
                for (SequenceFeature sf: convertedDocument.getIdentifieds(URI_prefix, SequenceFeature.class)){
                    if(sf.getName().equals(comp.getName()))
                    {
                        String naString = getNaStringFromSequenceFeature(sf,convertedDocument.getSequences().get(0));
                        Sequence sequence=newDocument.createSequence(comp.getDisplayId()+"_sequence");
                        comp.setSequences(sequence);
                    }
            ...
            // CHOICE 1
            List<Interaction> interaction_PhIF_PPhIF = SBOLAPI.createInteraction(Arrays.asList(InteractionType.Inhibition.getUri()), device, cds_PhIF, Arrays.asList(ParticipationRole.Inhibitor.getUri()), prm_PPhIF, Arrays.asList(ParticipationRole.Inhibited.getUri()));
            // CHOICE 2
            // Essentially same
    ...
    private Component createComponentWithSequence(String name, SBOLDocument newDocument, 
        ...
        */
        ...
        if(sf == null) {
            ...
            SubComponent sc = device.createSubComponent(comp);
        ...
        //String na = findSequence(convertedDocument, name);

        return createSubCompAndNaString(newDocument, sf, c_seq, convertedDocument.getSequences().get(0), comp, device);
        
        //String na = getNaStringFromSequenceFeature(sf, convertedDocument.getSequences().get(0));
    ...
    private Component createSubCompAndNaString(SBOLDocument newDocument, SequenceFeature sf, Sequence sequence, Sequence convDocSequence, Component comp, Component device) throws SBOLGraphException {
        ...
        // START AND END ARE INCLUSIVE AND 1-BASED INDEXING, SO SUBSTRING SHOULD BE (START-1, END)
        //String naString = convDocSequence.getElements().substring(start-1, end);
        //System.out.println(comp.getName() + " Start: " + start + ", End: " + end + ", Length: " + naString.length() + " Difference " + ((end-start)+1));

        /* 
        // Optionally set the sequence elements if na is not null
        if (naString != null) 
            sequence.setElements(naString);
        ...
        */
        ...
        sc.createSourceRange(start, end, device.getSequences().get(0));
    ...
    /*private String getNaStringFromSequenceFeature(SequenceFeature sf, Sequence sequence) throws SBOLGraphException {
        // Extract nucleotide string from SequenceFeature's locations
        if (sf == null || sequence == null) return null;
        String elements = sequence.getElements();
        if (elements == null) return null;
        StringBuilder sb = new StringBuilder();
        for (Location loc : sf.getLocations()) {
            // Assume RangeLocation (1-based inclusive)
            Range rangeLoc = (Range) loc; 
            int start = rangeLoc.getStart().get();
            int end = rangeLoc.getEnd().get();
            // Defensive: check bounds
            if (start > 0 && end > 0 && end <= elements.length() && start <= end) {
                sb.append(elements.substring(start - 1, end));
            }
        }
        return sb.toString();
    }*/
